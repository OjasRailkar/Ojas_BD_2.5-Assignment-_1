...
function sortedProductsByPopularity(product1, product2) {
  return product2.rating - product1.rating; //High to Low(Desc)
}

app.get('/products/sort/popularity', (req, res) => {
  let productsCopy = products.slice();
  let sortedProducts = productsCopy.sort(sortedProductsByPopularity);
  res.json({ product: sortedProducts });
});

//Endpoint 2: Get the products sorted by “high-to-low” price
function sortProductByPriceDescending(product1, product2) {
  return product2.price - product1.price;
}

app.get('/products/sort/price-high-to-low', (req, res) => {
  let productsCopy = products.slice();
  let sortedProducts = productsCopy.sort(sortProductByPriceDescending);
  res.json({ product: sortedProducts });
});

//Endpoint 3: Get the products sorted by “low-to-high” price
function sortProductByPriceAscending(product1, product2) {
  return product1.price - product2.price;
}

app.get('/products/sort/price-low-to-high', (req, res) => {
  let productsCopy = products.slice();
  let sortedProducts = productsCopy.sort(sortProductByPriceAscending);
  res.json({ product: sortedProducts });
});
